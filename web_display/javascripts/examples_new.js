$(document).ready(function() {
    ...
    */
    ...
    function showVideos (counter){
        ...
        var videosCurrent = videoFileGeneral.concat(pctnum, '_pct.json');
        console.log('videos current: ' + videosCurrent);
    ...
    $("#next").click(function() {
        ...
        if ($exampleNumber.text() <= perblock) {
            $exampleNumber.text(+$exampleNumber.text() + 1);

            //Show the array of the data (debugging)
            console.log($exampleNumber.text())
        }

        // When the videos reach the end of the block, go to next block
        if ($exampleNumber.text() == perblock+1) {
            // When the last block is over, user is moved to 'thank you' page
            if (pctnum == "100") {
                window.open("thanks.html");
            }
            else {
                next_pct = (parseInt(pctnum) + 25).toString();
                next_window = "triplet_examples1.html?pctnum=".concat(next_pct);
